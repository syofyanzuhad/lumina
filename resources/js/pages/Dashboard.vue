...
import { Eye, Users, Globe, Code, Calendar, Sparkles, RefreshCw, Smartphone, Laptop, Monitor, Download, Maximize2, CalendarDays, Filter, ExternalLink } from '@lucide/vue';
        ...
        <template v-if="!activeTab || activeTab === 'overview'">
                    ...
                    <div class="space-y-2">
                        ...
                        >
                            ...
                            <span class="relative z-10 truncate font-mono text-foreground font-medium group-hover:text-indigo-700 dark:group-hover:text-indigo-300 transition-colors mr-2 flex items-center gap-1 min-w-0">
                                ...
                                <a
                                    :href="`https://${activeSite.domain}${page.path.startsWith('/') ? '' : '/'}${page.path}`"
                                    target="_blank"
            ...
            <div v-if="overview.utm_campaigns && overview.utm_campaigns.length > 0" class="group/card rounded-xl border border-sidebar-border/70 dark:border-sidebar-border bg-card p-6 shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-muted-foreground font-mono">{{ Math.min(10, overview.utm_campaigns.length) }} of {{ overview.utm_campaigns.length }} campaigns</span>
                        <button
            ...
            <SheetContent side="right" class="w-full sm:max-w-lg overflow-y-auto p-4 sm:p-5">
                <SheetHeader class="pb-2 border-b border-sidebar-border/50">
                    ...
                    <SheetDescription class="text-xs text-muted-foreground">
                        Complete detailed breakdown for <strong class="text-foreground">{{ activeSite.domain }}</strong>
                    </SheetDescription>
                </SheetHeader>
